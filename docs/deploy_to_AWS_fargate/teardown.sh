#!/bin/bash
# =============================================================================
# teardown.sh — POC-Tracker Complete AWS Resource Cleanup
# =============================================================================
# Usage:  ./teardown.sh
# Reads resource IDs from .poc-tracker-state (created by deploy.sh)
# =============================================================================
...
echo ""
echo -e "${BOLD}${RED}═══════════════════════════════════════════════════${NC}"
echo -e "${BOLD}${RED}  POC-Tracker — Complete Teardown${NC}"
echo -e "${BOLD}${RED}═══════════════════════════════════════════════════${NC}"
echo ""
...
# MCP target group (present only on MCP deployments). Resolve by name if the state
# file predates MCP support, so teardown still cleans it up.
MCP_TG_ARN="${MCP_TG_ARN:-}"
if [ -z "$MCP_TG_ARN" ] || [ "$MCP_TG_ARN" = "None" ]; then
  MCP_TG_ARN=$(aws elbv2 describe-target-groups --names "${APP_NAME}-mcp-tg" \
    --query 'TargetGroups[0].TargetGroupArn' --output text --region "$REGION" 2>/dev/null || echo "")
fi
if [ -n "$MCP_TG_ARN" ] && [ "$MCP_TG_ARN" != "None" ]; then
  log "Deleting MCP target group..."
  aws elbv2 delete-target-group \
    --target-group-arn "$MCP_TG_ARN" \
    --region "$REGION" >/dev/null 2>/dev/null || warn "MCP target group not found — skipping"
  success "MCP target group deleted"
else
  skip "No MCP target group found"
fi
