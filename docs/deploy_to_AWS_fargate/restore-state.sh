#!/bin/bash
# =============================================================================
# restore-state.sh — Rebuild .poc-tracker-state from live AWS resources
# =============================================================================
...
# then writes a .poc-tracker-state file identical to the one deploy.sh produces.
...
# The default instance keeps the original ".poc-tracker-state" name; other instances
# are namespaced ".poc-tracker-state.<instance>" so several can coexist.
state_file_for() {
...
DEFAULT_INSTANCE="${INSTANCE:-poc-tracker}"
if [ -n "${INSTANCE:-}" ]; then
  APP_NAME="$INSTANCE"
else
...
# MCP target group — its presence tells us this was an MCP deployment
MCP_TG_ARN=$(aws elbv2 describe-target-groups \
  --names "${APP_NAME}-mcp-tg" \
  --query 'TargetGroups[0].TargetGroupArn' --output text --region "$REGION" 2>/dev/null || echo "")
[ "$MCP_TG_ARN" = "None" ] && MCP_TG_ARN=""
...
if [ -n "$ALB_ARN" ]; then
  ...
  if [ -n "$CERT_ARN" ] && [ "$CERT_ARN" != "None" ]; then
    ...
    CERT_ARN=""
    success "HTTPS not enabled (HTTP-only deployment)"
  fi
fi
...
cat > "$STATE_FILE" <<EOF
# POC-Tracker deployment state — regenerated by restore-state.sh
...
# Only record CONTAINER_IMAGE if the ECR repo exists (matches fix-image.sh behaviour)
