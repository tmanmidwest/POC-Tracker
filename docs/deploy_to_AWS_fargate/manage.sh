#!/bin/bash
# =============================================================================
# manage.sh — POC-Tracker Day-to-Day Management
# =============================================================================
# Usage:
...
if [ "${ENABLE_HTTPS:-false}" = "true" ] && [ -n "${DOMAIN_NAME:-}" ]; then
  ...
  MCP_SCHEME="https"; MCP_HOST="${DOMAIN_NAME}"
else
...
case "$CMD" in
  ...
  status)
    echo ""
    echo -e "${BOLD}  POC-Tracker — Status${NC}"
    echo -e "  ─────────────────────────────────────────"
  ...
  stop)
    echo ""
    log "Stopping POC-Tracker (setting desired count to 0)..."
  ...
  help|*)
    echo ""
    echo -e "${BOLD}  POC-Tracker — Management Commands${NC}"
    echo ""
