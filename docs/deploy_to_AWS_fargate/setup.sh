#!/bin/bash
# =============================================================================
# setup.sh — Pre-deployment prerequisite checker for POC-Tracker
# =============================================================================
...
echo ""
echo -e "${BOLD}${BLUE}  POC-Tracker — Setup Checker${NC}"
...
# docker buildx (required for linux/amd64 builds on Apple Silicon / ARM)
...
# Python3 (used for parsing AWS CLI JSON output in scripts)
...
aws ecs list-clusters --region "${REGION:-us-east-1}" >/dev/null 2>&1 \
  ...
  || { fail "No ECS access — your AWS user may need ECS permissions"; PERMS_OK=false; }

# ECR
aws ecr describe-repositories --region "${REGION:-us-east-1}" >/dev/null 2>&1 \
  && pass "ECR access confirmed" \
  || { fail "No ECR access — your AWS user may need ECR permissions"; PERMS_OK=false; }

# EFS
aws efs describe-file-systems --region "${REGION:-us-east-1}" >/dev/null 2>&1 \
  && pass "EFS access confirmed" \
  || { fail "No EFS access — your AWS user may need EFS permissions"; PERMS_OK=false; }
...
aws ec2 describe-vpcs --region "${REGION:-us-east-1}" >/dev/null 2>&1 \
  ...
  || { fail "No EC2 access — your AWS user may need EC2 permissions"; PERMS_OK=false; }

# ELB
aws elbv2 describe-load-balancers --region "${REGION:-us-east-1}" >/dev/null 2>&1 \
  && pass "Elastic Load Balancing access confirmed" \
  || { fail "No ELB access — your AWS user may need ELB permissions"; PERMS_OK=false; }
...
if curl -sf "https://github.com/tmanmidwest/POC-Tracker" >/dev/null 2>&1; then
  pass "GitHub repo is reachable (https://github.com/tmanmidwest/POC-Tracker)"
else
