...
> **Two traps that will silently break the cutover (both hit on the TST run):**
>
> 1. **Push before you deploy.** `deploy.sh`/`update.sh` build the image by cloning
>    **GitHub `main`**, *not* your local folder. A local-only commit deploys stale
>    (SQLite) code — which then runs 2 tasks on one SQLite file → `disk I/O error`.
>    Make sure the Postgres commit is **pushed to GitHub** first.
> 2. **The Postgres schema must exist before you migrate.** The migration script
>    loads into an already-migrated database. If you run it before the new app has
>    booted against RDS (B.2 completed with the *correct* image), it fails its
>    schema-revision check and loads nothing — and the app then seeds itself fresh
>    (blank SMTP/SSO, default admin). Always finish B.2 and confirm the app is up on
>    Postgres, *then* run B.3.
